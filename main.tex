...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{img/imagenProcesada.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.3\textwidth]{img/notificacionTelegram.png}
...
\begin{itemize}
  \item Imágenes capturadas: 131
  \item Detecciones exitosas: ~90\%
  \item Falsas activaciones: ~10\%
  \item Tiempo de operación sin fallos: 7 horas en dos intervalos, el primero de 4 horas y el segundo de 3 horas.
\end{itemize}

Además de detectar animales, las pruebas de campo demostraron la eficacia del sistema como herramienta de seguridad y vigilancia perimetral. La Figura~\ref{fig:ejemplos-deteccion-campo} ilustra detecciones exitosas de intrusos y vehículos no autorizados en el predio.
...
La latencia de extremo a extremo (desde la captura de la imagen en el nodo de cámara hasta su recepción en el servidor) depende de la interacción de múltiples factores:

\begin{itemize}
  \item \textbf{Calidad de la señal Wi-Fi:} El nivel de RSSI entre cada nodo y su padre influye directamente en la tasa de transmisión efectiva y en la probabilidad de retransmisiones.
  \item \textbf{Ancho de banda del punto de acceso:} La velocidad de enlace proporcionada por el router o punto de acceso al que se conecta el nodo raíz condiciona el cuello de botella hacia internet.
  \item \textbf{Profundidad de la red:} Cada salto adicional en la topología de árbol introduce latencia, ya que el paquete debe ser reenviado en la capa de red por cada nodo intermedio.
  \item \textbf{Tamaño del paquete:} Las imágenes \gls{jpeg} transmitidas oscilan entre \qty{30}{KB} y \qty{35}{KB}, un volumen significativo para enlaces Wi-Fi de baja potencia como la Red Mesh.
\end{itemize}

% Es importante destacar que, durante las pruebas de campo, el nodo raíz se conectó a un \textit{hotspot} de un teléfono celular en lugar de un router Wi-Fi dedicado. Esta configuración, si bien permitió validar el funcionamiento del sistema en condiciones reales, no representa el escenario óptimo de conectividad. Un router fijo con antena externa ofrecería un enlace más estable y de mayor ancho de banda, mejorando potencialmente los tiempos de transmisión.

\newpage
% TODO: Completar con valores medidos
\begin{table}[htbp]
  \centering
  \caption{Latencia de transmisión medida (con \textit{hotspot} celular).}
  \label{tab:latencia}
  \begin{tabular}{lc}
    \hline
    \textbf{Métrica}    & \textbf{Valor} \\
    \hline
    Latencia mínima     & 20 \unit{s}   \\
    Latencia promedio   & 40 \unit{s}   \\
    Latencia máxima     & 60 \unit{s}   \\
    Desviación estándar & 10 \unit{s}   \\
    \hline
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  \caption{Consumo energético del nodo hoja en modo de ultra bajo consumo.}
...
En consecuencia, la capacidad efectivamente utilizable del pack es inferior a los \qty{3500}{mAh} nominales. Considerando la curva de descarga típica de las celdas 18650~\cite{powar2025discharge18650} y el corte práctico en \qty{3.25}{V} por celda, la capacidad aprovechable se reduce a aproximadamente un \qty{40}{\percent} de la nominal. La Figura~\ref{fig:curva-descarga} muestra la curva de descarga obtenida experimentalmente con las celdas utilizadas en el prototipo, donde se evidencia la caída pronunciada de tensión una vez superado el \qty{60}{\percent} de la capacidad.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{img/curvaDescargaBateria.png}
  \caption{Curva de descarga de las celdas 18650 utilizadas en el prototipo.}
...
\begin{description}
  ...
  \item[Cobertura de red:] La red mesh operó de forma estable con distancias de hasta \qty{18}{m} en línea de vista directa y \qty{12}{m} con vegetación intermedia. Estas distancias, aunque limitadas por las antenas integradas de baja ganancia del ESP32-CAM, son suficientes para un despliegue en cadena con nodos intermedios (Relay).
...
\begin{itemize}
  \item \textbf{Sensor PIR domótico:} El HC-SR501 está optimizado para la detección de personas. Su sensibilidad puede ser insuficiente para fauna de menor porte o animales de sangre fría, lo que podría generar falsos negativos en escenarios de monitoreo biológico puro.
  \item \textbf{Alcance limitado por antena integrada:} La antena PCB del módulo ESP32-CAM restringe el rango efectivo de comunicación. En vegetación densa, las distancias se reducen considerablemente, requiriendo mayor densidad de nodos intermedios.
  \item \textbf{Brownout por agotamiento de batería:} Cuando la tensión del pack 2S desciende por debajo de \qtyrange{6}{7}{V}, el regulador LM2596 no puede compensar los picos de corriente del módulo Wi-Fi, provocando reinicios por \textit{brownout} que interrumpen la operación del nodo.
  \item \textbf{Ausencia de visión nocturna:} El sensor OV2640 no cuenta con iluminación infrarroja, limitando la captura de imágenes útiles a condiciones de luz diurna o crepuscular.
\end{itemize}
