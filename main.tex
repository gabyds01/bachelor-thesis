...
El sistema de notificaciones se implementa a través de un bot de la plataforma Telegram, empleando la librería \texttt{python-telegram-bot}. Este componente opera de manera asíncrona tras finalizar el procesamiento de cada imagen. Cuando el sistema clasifica objetos de interés que superan el nivel de confianza prestablecido, el bot genera y transmite la alerta correspondiente.

Cada notificación enviada por el bot estructura la información de la siguiente manera:

\begin{itemize}
	\item La imagen original en alta definición capturada por el nodo de la red mesh.
	\item La imagen anotada evidenciando las \glspl{bounding-box} de cada una de las detecciones realizadas por \gls{speciesnet}.
	\item Un resumen textual con la cantidad total de objetos detectados y sus respectivas métricas de confianza porcentual.
	\item Las principales etiquetas de clasificación taxonómica (top 5), en caso de tratarse de especies de fauna, organizadas de mayor a menor confianza.
\end{itemize}

El bot cuenta con comandos como \texttt{/last} que permiten a los usuarios consultar interactivamente la información y los análisis del último evento registrado por el servidor.
